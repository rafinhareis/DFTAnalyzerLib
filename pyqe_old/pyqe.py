...
class File:  # this class is relative to treatement of the raw data in to a better analize format
    ...
    def Load(self):
        ...
        try:
            self.fermi_energy = E_fermi(self.scfout_file,self.scfout_file_name,self.version_QE)
        except UnboundLocalError:
            try:
                self.fermi_energy = E_fermi(self.scfout_file,self.nscfout_file_name,self.version_QE)
            except UnboundLocalError:
                    self.fermi_energy = E_fermi(self.scfout_file,self.bandout_file_name ,self.version_QE)
    ...
    def Bands_files(self, fermi_energy_corr=True, bands_project = False):
        ...
        elif self.version_QE == '7.2' and self.verbosity == 'low':
                self.fermi_energy = E_fermi(self.bandout_file,'bands.out',self.version_QE)
    ...
    def Dos_files(self, calculation, fermi_energy_corr=True):  # dos via projwfc.x, nao usar dos.x
        ...
        if self.version_QE == '7.1' or self.version_QE == '7.2':
            self.fermi_energy = E_fermi(self.nscfout_file,self.nscfout_file_name,self.version_QE)
        ...
        for dir,subdir,files in os.walk(self.file_dir):
            for file in files:
                if ('pdos' in file) == True:
                    if file[:len(prefix)] == prefix and file != prefix+'.pdos.dat.pdos_tot' and file != prefix + '.pdos-proj.dat.projwfc_up' and file != prefix+'.pdos_tot' and  file !='pdos.dat.pdos_tot' and file != 'pdos_tot' and file !='pdos-proj.dat.projwfc_up':
                    ...
                    elif file =='pdos.dat.pdos_tot' or file == 'pdos_tot':
                        pdos_tot_file = os.path.join(self.file_dir,file)                       
...
class Plot:
    ...
    def kresolved(self, data, states = [0],subplotsize=(10, 8), subplot=False, ax=None, ylabel = 'E - E_f (eV)', E_min = -5, E_max = 5, dE = 1,loc = 'upper right',title = 'Band projected'
                  , cmap = 'magma', font_axis = 20, font_label = 20,cbar_label = ['0','1'], cbar_tick = [0,1],thresh = True, tmin= 0,tmax = 1):
        ...
        if thresh == False:
            ...
            im = ax.pcolormesh(K,E,dos, cmap=cmap, shading='auto',vmin = tmin,vmax=tmax)
